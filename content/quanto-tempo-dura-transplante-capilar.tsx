...
export default function Article() {
  return (
    <>
      ...
      <P>{"Cinco variáveis, principalmente, definem se uma cirurgia vai durar 6 ou 12 horas (ou mais):"}</P>
      <OL>
      ...
      <OL>
        ...
        <LI><Strong>{"Intervalo:"}</Strong>{" costuma haver uma pausa entre extração e implantação para alimentação leve e descanso do paciente, quando não é feita sedação venosa, já que o procedimento é longo. Com sedação venosa não há intervalos."}</LI>
      ...
      <P>{"Passar de 8 a 12 horas em uma sala cirúrgica soa mais desgastante do que costuma ser na prática. Com sedação venosa bem ajustada, os pacientes dormem durante toda a cirurgia. O ponto central, de qualquer forma, é que a sedação venosa foi pensada exatamente para que essas horas não sejam sentidas como um desconforto."}</P>

      <H2 id="a-duracao-influencia-o-resultado">{"Uma cirurgia mais longa significa um resultado melhor?"}</H2>
      <P>{"Não necessariamente, e essa é uma confusão comum. A duração reflete principalmente a quantidade de folículos e a complexidade do planejamento, não a qualidade técnica em si. Uma cirurgia mais rápida do que a média para o volume de folículos planejado pode até ser um sinal de alerta, porque pressa na extração tende a aumentar a taxa de transecção total e parcial, o que compromete a sobrevivência do enxerto. O objetivo nunca é terminar rápido, é terminar dentro do tempo necessário para preservar cada unidade folicular extraída."}</P>

      <H2 id="perguntas-frequentes">{"Perguntas frequentes"}</H2>
      <H3>{"Existe um número fixo de horas para o transplante capilar?"}</H3>
      <P>{"Não. A faixa mais comum fica entre 8 e 12 horas, mas cirurgias com grande quantidade de folículos ou planejamento mais complexo podem ultrapassar esse intervalo. O tempo exato só é definido depois da avaliação individual."}</P>
      ...
      <P>{"Depende. Se for sedação venosa há necessidade de jujeum prévio. Caso seja com sedação oral, costuma haver um intervalo para alimentação leve entre a etapa de extração e a de implantação. As orientações específicas de jejum pré-operatório e alimentação no dia seguem o protocolo passado na avaliação e na equipe de anestesiologia."}</P>
      <H3>{"O acompanhante pode ficar esperando durante toda a cirurgia?"}</H3>
      <P>{"A estrutura de acompanhamento varia conforme a clínica. O ideal é confirmar esse ponto diretamente na avaliação, já que envolve a logística da clínica e do centro cirúrgico."}</P>
