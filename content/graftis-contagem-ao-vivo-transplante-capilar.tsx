...
export default function Article() {
  return (
    <>
      ...
      <P>{"A extração é dividida em quatro quadrantes (temporal direito, temporal esquerdo, occipital direito e occipital esquerdo), a mesma lógica usada na avaliação da área doadora. Cada folículo extraído é classificado no ato: quantos fios tem, se veio íntegro ou com algum grau de transecção. O sistema soma tudo automaticamente e mostra, a qualquer momento, o total geral e o total de cada quadrante separadamente, sem que ninguém precise parar a cirurgia para fazer conta."}</P>
      <P>{"Isso também vale para as pré-incisões na área receptora: cada região (linha frontal, entradas, topete, coroa, escalpo médio) tem seu próprio contador, e o app soma tudo para comparar depois com o total extraído. Essa comparação (pré-incisões abertas contra folículos efetivamente implantados) é um dos números que uso para avaliar se o planejamento da área receptora bateu com a extração real daquele dia."}</P>
      <P>{"Cada auxiliar acessa a cirurgia pelo próprio celular ou num tablet (Ipad) disponível pela clínica, só com o link daquele caso específico, sem precisar de cadastro. Quem organiza a agenda cirúrgica (no meu caso, eu) tem login próprio e só enxerga as próprias cirurgias, nunca as de outro médico que eventualmente use o mesmo sistema. Isso permite ter três, quatro celulares registrando ao mesmo tempo durante uma extração longa, todos vendo o mesmo total atualizado a cada poucos segundos, sem planilha compartilhada nem anotação em papel para depois consolidar."}</P>
      <Figure
      ...
      <P>{"Além da contagem manual da equipe, o extrator utilizado (Mamba) dá uma leitura própria da extração. O Graftis registra essa leitura por quadrante e calcula, sozinho, a diferença entre o que o aparelho leu e o que a equipe contou à mão naquele mesmo quadrante. Divergência grande entre os dois números é um sinal de alerta que vale investigar ainda durante a cirurgia, não só depois que ela termina. Inclusive temos dentro do APP alertas de transecção parcial e total customizáveis."}</P>
      <P>{"O sistema também calcula o ritmo de extração em folículos por hora, de duas formas: pela contagem manual e pela leitura do aparelho. Como cada médico extrai numa ordem diferente de quadrantes, o Graftis usa o horário real de cada marcação para descobrir sozinho qual quadrante veio antes de qual, em vez de assumir uma ordem fixa."}</P>
      <Figure
      ...
      <P>{"Ao finalizar, a cirurgia inteira fica resumida numa única tela: tempo de extração, tempo de pré-incisões, tempo total do procedimento, folículos por categoria, taxas de transecção e a diferença final entre pré-incisões e extração. Esse resumo pode ser salvo como PDF e faz parte do prontuário daquele caso."}</P>
      <P>{"Como cada cirurgia fica registrada, também consigo acompanhar indicadores ao longo do tempo, comparando o volume médio de folículos por cirurgia, o índice de fios por folículo e as taxas de transecção entre casos. Isso ajuda a manter um padrão de qualidade consistente, em vez de avaliar cada cirurgia isoladamente."}</P>
      <Figure
      ...
      <P>{"Qualquer correção feita nos números durante a cirurgia, como reabrir um quadrante já fechado para ajustar um número específico de folículos, já recalcula sozinho todos os totais que dependem dele, sem precisar refazer conta nenhuma na mão."}</P>

      <H2 id="por-que-importa">{"Por que isso importa para você"}</H2>
      <P>{"Esse nível de controle não muda o resultado biológico do transplante, que continua dependendo da sua área doadora, da técnica cirúrgica e da cicatrização individual de cada paciente, e varia de pessoa para pessoa. O que ele muda é a base de decisão durante a cirurgia: com números conferidos em tempo real, dá para ajustar a estratégia de extração e distribuição no mesmo dia, em vez de descobrir uma divergência só na revisão do dia seguinte."}</P>
      <UL>
        <LI><Strong>{"Menos erro de soma manual"}</Strong>{", porque a conta é sempre automática."}</LI>
        <LI><Strong>{"Auditoria cruzada em tempo real"}</Strong>{" entre a contagem da equipe e a leitura do aparelho."}</LI>
        <LI><Strong>{"Comparação objetiva"}</Strong>{" entre o planejamento da área receptora e o que foi de fato extraído."}</LI>
