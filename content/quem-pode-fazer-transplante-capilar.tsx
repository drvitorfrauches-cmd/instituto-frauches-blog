...
export default function Article() {
  return (
    <>
      ...
      <P>{"O padrão e o estágio da calvície (avaliados por escalas como a de Norwood, no caso masculino) ajudam a definir se o transplante já é indicado, e quantas sessões podem ser necessárias. Calvícies muito avançadas, com pouca área doadora disponível em relação à área a cobrir, exigem um planejamento mais cauteloso: às vezes o objetivo realista é reconstruir a região frontal com prioridade, não cobrir 100% do couro cabeludo numa única cirurgia."}</P>

      <H2 id="area-doadora">{"Área doadora"}</H2>
      <P>{"A área doadora, geralmente a região posterior e lateral da cabeça, é o que viabiliza tecnicamente a cirurgia. Pacientes com pouca densidade nessa região, seja por herança genética, seja por transplantes anteriores malfeitos, têm menos folículos disponíveis para transplantar, o que limita o resultado possível. Avaliar a qualidade e a densidade da área doadora é uma das primeiras coisas que fazemos numa consulta."}</P>

      <H2 id="mulheres">{"Mulheres"}</H2>
      <P>{"Mulheres também podem ser candidatas a transplante capilar, principalmente em casos de perda de densidade frontal ou difusa. A diferença é que a calvície feminina costuma ter um padrão mais difuso (perda espalhada, não concentrada em entradas ou coroa), o que exige uma avaliação minuciosa da causa da queda antes de indicar cirurgia, já que fatores hormonais e nutricionais têm um peso maior nesses casos."}</P>

      <H2 id="casos-que-devem-tratar-antes">{"Casos que devem tratar a queda antes de operar"}</H2>
      <P>{"Existe uma diferença importante entre calvície estabilizada e queda ativa. Quando o paciente ainda está perdendo cabelo nativo de forma acelerada, geralmente por alopecia androgenética não tratada, transplantar antes de controlar essa queda é um risco: os fios nativos ao redor da área transplantada continuam caindo, e o resultado visual piora com o tempo, mesmo que a cirurgia em si tenha sido bem-feita. Por isso, em boa parte dos casos, o primeiro passo é estabilizar a queda com tratamento clínico e só depois planejar a cirurgia."}</P>
      <UL>
      ...
      <P>{"Ser tecnicamente elegível para a cirurgia não é a mesma coisa que ter uma expectativa compatível com o que a área doadora consegue entregar. Parte importante da consulta é justamente alinhar isso: se o paciente busca densidade máxima numa região extensa, mas a área doadora disponível é limitada, o resultado real vai ficar aquém do imaginado, mesmo com uma cirurgia tecnicamente bem executada. Por isso, um bom atendimento inclui explicar com clareza o que é fisicamente possível antes de qualquer cirurgia ser marcada, em vez de simplesmente aceitar qualquer expectativa do paciente."}</P>

      <H2 id="tipos-de-fio">{"O tipo de fio também entra na avaliação"}</H2>
      <P>{"Cabelos lisos, ondulados, cacheados e afro têm comportamentos diferentes durante a extração e a implantação, o que influencia o planejamento cirúrgico. Fios muito cacheados ou crespos, por exemplo, costumam ter uma curvatura sob a pele que exige mais atenção na hora da extração, para não danificar o folículo. Isso não impede a cirurgia, mas reforça por que a experiência da equipe com diferentes tipos de cabelo é um critério relevante na escolha de onde operar, principalmente para pacientes com fios cacheados ou afro."}</P>
      ...
      <P>{"A forma mais segura de saber se você é candidato é uma avaliação médica individual, que já leva em conta seu histórico e o estágio atual da sua calvície. Se você já pesquisou sobre "}<Link href="/blog/transplante-capilar-fue-o-que-e" className="underline">{"como funciona a técnica FUE"}</Link>{" e quer saber se o seu caso se encaixa, "}<a href={WHATSAPP_URL} className="underline" target="_blank" rel="noopener noreferrer"><Strong>{"agende sua avaliação pelo WhatsApp"}</Strong></a>{"."}</P>
